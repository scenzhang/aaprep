...
def array_sum_with_index(arr)
  arr.each_with_index.reduce(0) { |acc, (n, i)| acc + n * i }
end
...
def price_is_right(bids, actual_retail_price)
  bids.reduce do |closest, bid|
    this_diff = actual_retail_price - bid
    if this_diff < actual_retail_price - closest && this_diff > 0
      bid
    else
      closest
    end
...
def num_factors(number)
  (1..number).each.reduce do |num_f, n|
    if number % n == 0
      num_f + 1
    else
      num_f
...
def ordered_vowel_words(words)
  words.select { |w| ordered_vowel_word?(w) }
end

def ordered_vowel_word?(word)
  vowels = ['a', 'e', 'i', 'o', 'u']
  word_vowels = word.chars.select { |c| vowels.include?(c) }
  word_vowels.reduce do |cur_vowel, c|
    if cur_vowel <= c
      c
    else
      return false
...
def products_except_me(numbers)
  numbers.each_index.map do |i|
    nums_cpy = numbers.clone
    nums_cpy.delete_at(i)
    array_product(nums_cpy)
  end
end
