...
describe MyHashSet do
  ...
  describe "set interaction methods" do
    ...
    describe "#symmetric_difference" do
      ...
      it "returns a set containing elements found in either of the sets but not both" do
    ...
    describe "#==" do
      it "returns false if obj is not a MyHashSet" do
        expect(set1 == []).to be_falsey
      end
      it "returns false if lengths are not the same" do
        set2.delete("Natalie Portman")
        expect(set1 == set2).to be_falsey
      end
      it "returns false if the sets are not the same" do
        expect(set1 == set2).to be_falsey
      end
      it "returns true if the sets are the same" do
        set1.delete("Mark Hamill")
        set1.delete("Harrison Ford")
        set2.delete("Natalie Portman")
        set2.delete("Ewan McGregor")
        expect(set1 == set2).to be_truthy
      end
