...
class String
  def caesar(shift)
    self.chars.map do |c|
      c.ord + shift < 122 ? (c.ord + shift).chr : (c.ord + shift - 26).chr
    end.join("")
  end
end
...
class Hash
  def difference(other_hash)
    diff = self.reject { |k, _| other_hash.key?(k) }
    other_hash.reject { |k, _| self.key?(k) }.each do |k, v|
      diff[k] = v
    end
    diff
...
class Fixnum
  def stringify(base)
    pow = 0
    #"a".ord = 97 so adding 87 to n for n>10 gives 10=>"A" 11=>"B" etc
    digit_h = Hash[
      (0..15).to_a.map { |n| n < 10 ? [n, n.to_s] : [n, (n + 87).chr] }
    ]
    res_a = []
    while self / base ** pow != 0
      res_a.unshift((self / base**pow) % base)
      pow += 1
    end
    res_a.reduce("") do |res_s, n|
      res_s + digit_h[n]
    end
